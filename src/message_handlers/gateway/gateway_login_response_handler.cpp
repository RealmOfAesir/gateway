...
void gateway_login_response_handler::handle_message(std::unique_ptr<message<false> const> const &msg,
    ...
    if (auto response_msg = dynamic_cast<login_response_message<false> const *>(msg.get())) {
        ...
        if(response_msg->error_number != 0) {
            login_response_message<true> response{{false, 0, 0, 0}, 0, response_msg->error_number, response_msg->error_str};
            ...
            login_response_message<true> response{{false, 0, 0, 0}, 0, 0, ""};
            auto response_str = response.serialize();
            connection->get().ws->send(response_str.c_str(), response_str.length(), uWS::OpCode::TEXT);
        }
